# -*- coding: utf-8 -*-
"""
Created on Mon Aug  1 11:13:52 2022
@author: alpno
"""
...
dataForModel = data[(data['task'] == 'go') & (data['Response.rt'].notnull()) & (data['Response.corr'] == 1)]

# Remove trials without n-1 FP values (i.e., first of each block)
dataForModel = dataForModel[dataForModel['prevFP'].notnull()]
...
if SavePlots==True:    
    ...
    for iSub,Sub in enumerate(unique_IDs):
        ...
        # Sequential effect
        plt.figure()
        seqeff = sns.factorplot(x='foreperiod', y='Response.rt', hue='prevFP', data=dataSub,
...
# Sequential effects
summaryData=dataForModel.groupby(['ID','foreperiod','condition','prevFP'],as_index=False)[['Response.rt']].mean()
plt.figure()
sns.factorplot(x='foreperiod', y='Response.rt', hue='prevFP', data=summaryData, col='condition',
               palette=sns.color_palette(['blue','orange','green']))
