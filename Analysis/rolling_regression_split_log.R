...
# Changes:
# Uses logRT instead of inverse RT
#===================================================================#
...
#===================== 3. Get action coefs =======================
# Get coefficients for the first half of the experiment for participants
...
goData4  %>% mutate(logRT = ifelse(Acc==1, logRT, NA), 
                    logRT = ifelse(outlier==FALSE, logRT, NA),
                    logRT = ifelse(!is.na(numOneBackFP), logRT, NA))  %>% 
...
## Save the plot
ggsave(paste("./Plots/Rolling_regression/Split_dataset/external_beta_split_log_", win+1, ".tiff", sep = "")
...
## Apply lm with rolling window get beta-timecourses
goData5  %>% mutate(logRT = ifelse(Acc==1, logRT, NA), 
                    logRT = ifelse(outlier==FALSE, logRT, NA),
                    logRT = ifelse(!is.na(numOneBackFP), logRT, NA))  %>% 
...
bigcoeffAction  %>% write.csv('./Analysis/lm_coeff_action_split_log.csv')
bigcoeffExternal  %>% write.csv('./Analysis/lm_coeff_external_split_log.csv')
