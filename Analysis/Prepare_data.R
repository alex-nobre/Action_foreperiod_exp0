...
data <- data %>%
  dplyr::select(ID, Acc, condition, block, trialType,
                foreperiod, RT, counterbalance, oneBackFP, twoBackFP,
                oneBacktrialType, twoBacktrialType, oneBackEffect)
...
# If FPn-1 is longer than the current FP, use the absolute value of this diff
# If FPn-1 is shorter than the current FP, set duration to zero
...
#oneBackFPDiff <- ifelse(oneBackFPDiff > 0, 1, ifelse(oneBackFPDiff < 0, -1, 0))
#oneBackFPDiff <- ifelse(oneBackFPDiff > 0, 0, oneBackFPDiff)
#oneBackFPDiff <- c(NA, oneBackFPDiff)
# oneBackFPDiff <- oneBackFPDiff %>%
#   as.factor() %>%
#   forcats::fct_relevel(c("0", "-0.6", "-1.2"))
# oneBackFPDiff <- oneBackFPDiff %>%
#  as.factor() %>%
#  forcats::fct_relevel(c("-1", "0", "1"))
# data$oneBackFPDiff <- as.factor(oneBackFPDiff)
# data$oneBackFPDiff <- as.factor(ifelse(oneBackFPDiff))
...
data <- data %>%
  ...
  mutate(oneBackFPDiff=forcats::fct_relevel(oneBackFPDiff, levels=c("no-go", "-1", "0", "1")))
