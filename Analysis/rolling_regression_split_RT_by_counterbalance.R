...
#===================== 3. Get action coefs =======================
# Get coefficients for participants in the "action-external" counterbalancing order
...
bigcoeffActionExternal <- bigcoeff %>%
  melt(id.vars=c('ID','trial')) %>% 
  group_by(ID, variable) %>%
  mutate(value = value - value[1]) %>%
  ungroup()

colnames(bigcoeffActionExternal) <- c('ID',c('trial','variable'),'dv')

# Compute means and confidence intervals for each time point
bigcoeffActionExternal <- bigcoeffActionExternal %>%
...
cfPlotActionExternal <- bigcoeffActionExternal %>%
  ggplot(aes(x=trial,
             y=meanDV)) +
  ...
  labs(x = "Trial",
       y = "beta coefficient",
       title= paste('Action-external beta coefs/window size =', win + 1))

# Lines to separate blocks
for (i in seq(4)) {
  idx = (i-1)* 75
  cfPlotActionExternal <- cfPlotActionExternal + geom_vline(xintercept = idx, color='black', linetype='dotted', linewidth=0.15*lsz)
}
cfPlotActionExternal

# Save the plot
ggsave(paste("./Plots/Rolling_regression/Split_dataset/action_beta_counterbalance_rt_", win+1, ".tiff", sep = "")
       ,width = 7.5, height=12, units = 'cm')

#===================== 4. Get external coefs =======================
# Now we get coefficients for participants in the "external-action" counterbalancing order
...
# Reshape data to "long" format by ID and trial index and subtract first value from each coefficient
bigcoeffExternalAction <- bigcoeff %>%
  melt(id.vars=c('ID','trial')) %>% 
  group_by(ID, variable) %>%
  mutate(value = value - value[1]) %>%
  ungroup()

colnames(bigcoeffExternalAction) <- c('ID',c('trial','variable'),'dv')

# Compute means and confidence intervals for each time point
bigcoeffExternalAction <- bigcoeffExternalAction %>%
  group_by(trial, variable) %>%
  summarise(meanDV = mean(dv, na.rm = TRUE),
            ciUpper = mean(dv, na.rm=TRUE) + 1.96 * (sd(dv, na.rm = TRUE)/sqrt(n())),
            ciLower = mean(dv, na.rm=TRUE) - 1.96 * (sd(dv, na.rm = TRUE)/sqrt(n()))) %>% 
...
# Plot coefficients by predictor
cfPlotExternalAction <- bigcoeffExternalAction %>%
  ggplot(aes(x=trial,
             y=meanDV)) +
  ...
  labs(x = "Trial", 
       y = "beta coefficient", 
       title = paste('External-action beta coefs/window size =', win + 1))

# Lines to separate blocks
for (i in seq(4)) {
  idx = (i-1)* 75
  cfPlotExternalAction <- cfPlotExternalAction + geom_vline(xintercept = idx, color='black',linetype='dotted', linewidth=0.15*lsz)
}
cfPlotExternalAction

# Save the plot
ggsave(paste("./Plots/Rolling_regression/Split_dataset/external_beta_counterbalance_rt_", win+1, ".tiff", sep = "")
       ,width = 7.5, height=12, units = 'cm')


####### For the permutation test, the python packages mne, pandas and numpy are used:
bigcoeffActionExternal  %>% write.csv('./Analysis/lm_coeff_actionexternal.csv')
bigcoeffExternalAction  %>% write.csv('./Analysis/lm_coeff_externalaction.csv')
