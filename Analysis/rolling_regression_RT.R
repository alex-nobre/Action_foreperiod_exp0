...
goData3  %>% mutate(RT = ifelse(Acc==1,RT,NA), 
...
# bigcoeff$condition <- cond$condition
# 
# # Rename columns
# names(bigcoeff) <- c('ID','trial','intercept','fpMain','oneBackFPMain','interaction', 'condition')
...
# bigcoeffBase <- bigcoeff %>%
#   melt(id.vars=c('ID','trial', 'condition')) %>% 
#   group_by(ID, variable) %>%
#   mutate(value = value - value[1]) %>%
#   ungroup()
# 
# colnames(bigcoeffBase) <- c('ID',c('trial', 'condition', 'variable'),'dv')
# 
# # Compute means and confidence intervals for each time point
# bigcoeffBase <- bigcoeffBase %>%
#   group_by(trial, variable, condition) %>%
...
colnames(bigcoeffBase) <- c('ID',c('trial', 'variable'),'dv')
...
cfPlot <- bigcoeffBase %>%
  ...
  labs(x = "Trial",
       y = "beta coefficient",
       title = paste('beta coefs/window size =', win + 1))


# Lines to separate blocks
for (i in seq(4)) {
  idx = (i-1)* 75
  cfPlot <- cfPlot + geom_vline(xintercept = idx, color='black', linetype='dotted', linewidth=0.15*lsz)
}
cfPlot

# Save the plot
ggsave(paste("./Plots/Rolling_regression/beta_rt_", win+1, ".tiff", sep = "")
       ,width = 7.5, height=12, units = 'cm')

####### For the permutation test, the python packages mne, pandas and numpy are used:
bigcoeffBase  %>% write.csv('./Analysis/lm_coeff_rt.csv')
